...
默认输出是输入视频旁边的 `<视频名>-弹幕版.mp4`，很容易与上次的产物重名。
此时程序既不会直接报错，也不会默默覆盖，而是在终端问一次：
...
提示里带上体积与最后修改时间，是为了让你能判断那是自己的成片还是上次失败
留下的残骸 —— 0 字节的多半是后者。

**非交互环境不询问**：管道、重定向、CI 或由其他程序调用时 stdin 不是终端，
`input()` 要么立刻拿到 EOF、要么永久阻塞等待一个不会到来的输入。这两种都不
是期望行为，所以一律按「不覆盖」处理并提示加 `-f`，不会卡住你的脚本。
