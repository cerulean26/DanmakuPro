...
- **变帧率（VFR）素材可直接使用**：弹幕层与画面按时间轴对齐，渲染帧率取真实平均帧率（优先读容器 `nb_frames`，缺失时用 `ffprobe -count_packets` 实测），因此弹幕时间轴长度恒等于视频时长，既不漂移也不丢尾部弹幕。仅当全片帧率剧烈波动时弹幕运动可能略有顿挫，此时运行时会给出提示；若在意观感，可先 `ffmpeg -i in.flv -vsync cfr out.mp4` 转成 CFR 再压制（代价是丢帧或复制帧）。
